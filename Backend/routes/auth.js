...
router.post("/login", [body("email").isEmail(), body("password").exists()], async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(404).json({ msg: "Invalid credentials, Failed to find user" });
    console.log("User: ", user);

    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) return res.status(400).json({ msg: "Invalid credentials. Fialed to match" });
